    ...
    @Query("""
            select e.id employee_id,
                ...
                jsonb_agg(distinct
                    ...
                    else jsonb_build_object('hoursSpent', ts.hours_spent,
                ...
                jsonb_agg(
                    distinct (case when v.id is null then null else 
                        jsonb_build_object('startDate', v.start_date, 'endDate', v.end_date) end)
                ) vacations
            ...
            """)
    Flux<TimesheetSummaryView> summary(@Param("from") LocalDate from, @Param("to") LocalDate to,

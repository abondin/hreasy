...
COMMENT ON COLUMN alloc.resource_allocation.percent IS 'Explicit allocation percentage from 0 through 1000; absence is represented by no row';
...
CREATE TABLE alloc.resource_allocation_change (
    ...
    period integer NOT NULL,
    employee_id integer NOT NULL REFERENCES empl.employee (id),
    previous_percent smallint NULL CHECK (previous_percent >= 0 AND previous_percent <= 1000),
    new_percent smallint NULL CHECK (new_percent >= 0 AND new_percent <= 1000),
...
COMMENT ON COLUMN alloc.resource_allocation_change.previous_percent IS 'Percentage before the revision, null when absent';
COMMENT ON COLUMN alloc.resource_allocation_change.new_percent IS 'Percentage after the revision, null when removed';

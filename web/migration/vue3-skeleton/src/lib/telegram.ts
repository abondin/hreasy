...
const telegramFullAccountRegexp = /(https?:\/\/)?(www[.])?(telegram|t)(\.me\/)([a-zA-Z0-9_-]+)\/?$/;
const telegramAtOrSimpleAccountRegexp = /^@?([a-zA-Z0-9_-]+)\/?$/;
const validShortUsernameOrPhone = /^([a-zA-Z0-9_-]+|(\+[0-9]+))\/?$/;

export function extractTelegramAccount(input: string | null | undefined): string | undefined {
...
export function isShortTelegramUsernameOrPhoneValid(input: string | null | undefined): boolean {
  if (!input) {
    return true;
  }

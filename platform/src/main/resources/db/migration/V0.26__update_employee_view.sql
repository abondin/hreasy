...
create view v_employee_detailed as select e.*,
        ...
        left join (
select s.employee_id employee_id, STRING_AGG(CONCAT_WS('|$|',s.id, s.name, gr.id, gr.name, IsNull(r.cnt,0), IsNull(r.av,0)),'|#|') rating from skill s
 inner join skill_group gr on gr.id=s.id
 left join (
 	select skill_id skill_id, count(rating) cnt, avg(rating) av from skill_rating where deleted_at is null group by skill_id
 ) r on s.id = r.skill_id group by employee_id) sk

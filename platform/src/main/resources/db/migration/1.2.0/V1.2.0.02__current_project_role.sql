-- Employee role on current project
ALTER TABLE empl.employee ADD COLUMN IF NOT EXISTS
    current_project_role VARCHAR(1024) NULL;
COMMENT ON COLUMN empl.employee.current_project_role IS 'Employee role on project (Backend Developer, Designer, PM, etc)';

ALTER TABLE empl.employee_history ADD COLUMN IF NOT EXISTS
    current_project_role VARCHAR(1024) NULL;
COMMENT ON COLUMN empl.employee_history.current_project_role IS 'Employee role on project (Backend Developer, Designer, PM, etc)';

-- Add employee's role on current project
drop view empl.v_employee_detailed;
create view empl.v_employee_detailed as select e.*,

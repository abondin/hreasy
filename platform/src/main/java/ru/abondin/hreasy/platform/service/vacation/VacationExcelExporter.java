...
public class VacationExcelExporter {
    ...
    private XSSFSheet createMainSheet(XSSFWorkbook wb, Map<String, XSSFCellStyle> styles, VacationsExportBundle bundle) {
        ...
        for (var v : bundle.vacations.stream()
                .sorted(Comparator.comparing(VacationDto::getEmployeeDisplayName))
            ...
            cell.setCellValue(v.getStartDate());
            cell.setCellStyle(styles.get("dateStyle"));
            cell = row.createCell(4);
            cell.setCellValue(v.getEndDate());
            cell.setCellStyle(styles.get("dateStyle"));
    ...
    private void createTable(XSSFWorkbook wb, XSSFSheet sheet, int firstRowIndex, int lastRowIndex) {
        ...
        var reference = wb.getCreationHelper().createAreaReference(
                new CellReference(firstRowIndex, 0), new CellReference(lastRowIndex, 8));
        var table = sheet.createTable(reference);
        table.setName("vacations_table");

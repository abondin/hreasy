...
public interface OvertimeReportRepo extends R2dbcRepository<OvertimeReportEntry, Integer> {
    ...
    @Query("-- All information from report\n" +
            ...
            " (select sum(i.hours) from overtime_item i where i.report_id=r.id and i.deleted_at is null) as total_hours,\n" +
            "-- Last update of items\n" +
            " (select max(i.created_at) from overtime_item i where i.report_id=r.id and i.deleted_at is null) as last_update,\n" +
            "-- Last positive approval decision \n" +
            " (select max(a.decision_time) from overtime_approval_decision a where a.report_id=r.id \n" +
            ...
            " from overtime_report r where r.period = :period")
